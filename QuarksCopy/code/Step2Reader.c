...
BufferPointer readerAddChar(BufferPointer const readerPointer, q_char ch) {
	...
	if ((readerPointer->position.write * (q_int)sizeof(q_char)) < readerPointer->size) {
...
q_int readerPrint(BufferPointer const readerPointer) {
	...
	if (readerPointer == NULL || readerPointer->content == NULL) {
		return Q_FALSE; // Invalid reader or content
	}
	...
	readerPointer->position.write = charsPrinted;
...
q_int readerGetPosRead(BufferPointer const readerPointer) {
	...
	/* TO_DO: Return read */
	if (readerPointer->position.read >= 0 && readerPointer->position.read < readerPointer->size) {
...
q_int readerGetPosWrte(BufferPointer const readerPointer) {
	...
	/* TO_DO: Return wrte */
	if (readerPointer->position.write >= 0 && readerPointer->position.write < readerPointer->size) {
...
*************************************************************
*/
q_byte readerGetFlags(BufferPointer const readerPointer) {
	...
	/* TO_DO: Return flags */
	if (readerPointer->flags.isFull || readerPointer->flags.isEmpty || 
		readerPointer->flags.isRead || readerPointer->flags.isMoved) {
		return (q_byte)((readerPointer->flags.isFull << 3) | 
						(readerPointer->flags.isEmpty << 2) | 
						(readerPointer->flags.isRead << 1) | 
